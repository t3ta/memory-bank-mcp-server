...
describe('GetRecentBranchesUseCase', () => {
  ...
  it('should get recent branches with default limit', async () => {
    ...
    expect(result.branches[0].summary.recentChanges).toEqual(['Added authentication', 'Fixed validation']);
    ...
    expect(result.branches[1].summary.recentChanges).toEqual(['Identified issue', 'Started fixing']);
  ...
  it('should wrap repository errors as ApplicationError', async () => {
    ...
    await expect(useCase.execute({})).rejects.toThrow('Failed to get recent branches: Database connection failed');
