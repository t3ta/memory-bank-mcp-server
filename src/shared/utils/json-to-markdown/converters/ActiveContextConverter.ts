...
export class ActiveContextConverter extends BaseDocumentTypeConverter {
  ...
  protected convertContent(document: JsonDocument, builder: MarkdownBuilder): void {
    ...
    if (
      ...
      Array.isArray(content.recentChanges) &&
    ...
    if (
      content.activeDecisions &&
      Array.isArray(content.activeDecisions) &&
      content.activeDecisions.length > 0
    ) {
    ...
    if (
      ...
      Array.isArray(content.considerations) &&
