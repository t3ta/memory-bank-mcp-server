...
describe('UpdateTagIndexUseCase', () => {
  ...
  it('should throw DomainError if branch does not exist', async () => {
    ...
    await expect(useCase.execute({ branchName })).rejects.toThrow(`Branch "${branchName}" not found`);
  ...
  it('should handle repository errors when listing documents', async () => {
    ...
    await expect(useCase.execute({})).rejects.toThrow('Failed to update tag index: Failed to list documents');
  ...
  it('should handle repository errors when getting documents', async () => {
    ...
    // Act & Assert
    await expect(useCase.execute({})).rejects.toThrow(ApplicationError);
    await expect(useCase.execute({})).rejects.toThrow('Failed to update tag index: Failed to get document');
  });
