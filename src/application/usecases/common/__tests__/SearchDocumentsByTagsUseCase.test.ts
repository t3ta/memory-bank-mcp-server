...
const createTestDocument = (path: string, tags: string[], content: string = `# Test Document\n\nContent for ${path}`): MemoryDocument => {
...
describe('SearchDocumentsByTagsUseCase', () => {
  ...
  it('should throw ApplicationError if no tags provided', async () => {
    ...
    await expect(useCase.execute(input)).rejects.toThrow('At least one tag must be provided for search');
  ...
  it('should throw DomainError if branch does not exist', async () => {
    ...
    await expect(useCase.execute({ tags: searchTags, branchName })).rejects.toThrow(`Branch "${branchName}" not found`);
  ...
  it('should throw DomainError if tag is invalid', async () => {
    ...
    await expect(useCase.execute({ tags: searchTags })).rejects.toThrow('Tag must contain only lowercase letters, numbers, and hyphens');
  ...
  it('should handle repository errors', async () => {
    ...
    await expect(useCase.execute({ tags: searchTags })).rejects.toThrow('Failed to search documents by tags: Database connection failed');
