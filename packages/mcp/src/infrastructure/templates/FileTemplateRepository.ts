 ...
 */
// Removed duplicate comment end */
import path from 'path';
import fs from 'fs/promises';
import { ITemplateRepository } from '../../domain/templates/ITemplateRepository.js'; // Re-applying correct path
import { Template } from '../../domain/templates/Template.js'; // Re-applying correct path
import type { LanguageTextMap } from '../../domain/templates/Section.js'; // Import only type, Section removed
import { Language } from '../../domain/i18n/Language.js'; // Re-added Language import
// import { DomainError } from '../../shared/errors/DomainError.js'; // DomainError was already removed
...
export class FileTemplateRepository implements ITemplateRepository {
  ...
  async getTemplateAsMarkdown(
    id: string,
    language: Language, // Re-added language parameter to match interface
    variables?: Record<string, string>
  ): Promise<string> {
    // TODO: Remove this console log once language is actually used
    console.log(`Rendering template ${id} for language: ${language.code}`); // Dummy usage to avoid unused parameter error
    ...
    // let markdown = `# ${template.getName(language)}\n\n`; // Commented out due to Template class change
    let markdown = `# ${template.id}\n\n`; // Use id as fallback title

    // for (const section of template.sections) { // Commented out due to Template class change
  ...
  private async loadTemplate(id: string): Promise<void> {
    try {
      ...
      // Convert to domain model (currently unused due to Template class change)
  ...
  private async loadTemplateV1(data: TemplateFile, id: string): Promise<void> {
    try {
      ...
      // Create translation maps for each section (currently unused due to Template class change)
  ...
  private async saveTemplateToFile(template: Template): Promise<void> {
    try {
      ...
      // const sections: SectionData[] = template.sections.map(section => { // Commented out due to Template class change
      //   return {
      //     id: section.id,
      ...
      //   };
      // }); // End of commented out map

      const data: TemplateFile = {
        id: template.id,
        type: template.type,
        // names: template.nameMap, // Commented out due to Template class change
        // sections, // Commented out due to Template class change
