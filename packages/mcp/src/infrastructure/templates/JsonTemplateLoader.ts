...
import {
  Language,
  JsonDocumentV2Schema as JsonTemplate, // Use JsonDocumentV2Schema and alias it
  // validateJsonTemplate, // Removed validateJsonTemplate
  // BranchContextJsonV2Schema, // Removed unused import
  // ActiveContextJsonV2Schema, // Removed unused import
  // ProgressJsonV2Schema, // Removed unused import
  // SystemPatternsJsonV2Schema, // Removed unused import
  // GenericDocumentJsonV2Schema, // Removed unused import
...
export class JsonTemplateLoader implements ITemplateLoader {
  ...
  constructor(
    ...
    this.templateRenderer = new TemplateRenderer(/* i18nProvider */); // Removed argument
  }
  ...
  private getJsonTemplatesDirectory(): string {
    ...
    // Assuming templates are now within the package structure
    return path.join(process.cwd(), 'packages/mcp/src/templates/json'); // Adjusted path
  }
  ...
  private getLegacyTemplatesDirectory(): string {
    // Assuming legacy templates might also be within the package or a shared location
    return path.join(process.cwd(), 'packages/mcp/src/templates/markdown'); // Adjusted path
  }

  /**
   * Implements ITemplateLoader.loadJsonTemplate
   */
  async loadJsonTemplate(templateId: string): Promise<JsonDocumentV2> { // Use JsonDocumentV2 type
    ...
    try {
      ...
      const validationResult = JsonTemplate.safeParse(templateData);
      if (!validationResult.success) {
        // Add type ZodIssue to the map function parameter
        throw new Error(`Invalid template schema: ${validationResult.error.errors.map((e: import('zod').ZodIssue) => `${e.path.join('.')}: ${e.message}`).join(', ')}`);
      }
