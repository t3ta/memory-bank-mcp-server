...
// テスト用のドキュメント生成関数
function createSystemPatternsDocument(content: any = { 
  technicalDecisions: [{
    title: 'ダミー決定',
    context: 'ダミーコンテキスト',
    decision: 'ダミー決定内容',
...
describe('SystemPatternsConverter', () => {
  ...
  test('should convert complete system patterns document', () => {
    ...
    expect(markdown).toContain('リポジトリパターンを採用し、インターフェースを介してデータアクセスを行う');
  ...
  test('should handle decision with missing fields', () => {
    ...
    expect(markdown.indexOf('### タイトルのみの決定事項') < markdown.indexOf('### 部分的なフィールド'));
