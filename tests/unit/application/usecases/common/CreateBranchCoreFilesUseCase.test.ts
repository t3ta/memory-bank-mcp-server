...
/**
 * Unit tests for CreateBranchCoreFilesUseCase
 * 
 * These tests verify that the CreateBranchCoreFilesUseCase correctly implements:
 * - Creating all core JSON files (activeContext, progress, systemPatterns) in a branch
 * - Properly formatting these files with correct schema, metadata, and content structure
 * - Input validation for branch names and file data
 * - Proper error handling for non-existent branches
 * - Handling of empty lists in input data
 * 
 * The test uses mocked IBranchMemoryBankRepository to isolate the use case behavior.
 */
