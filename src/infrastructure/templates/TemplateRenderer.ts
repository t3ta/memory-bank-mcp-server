...
export class TemplateRenderer {
   ...
   * @returns Markdown formatted string
   */
  private renderBaseTemplateToMarkdown(template: BaseTemplate, language: Language, variables?: Record<string, string>): string {
    const lines: string[] = [];
   ...
   * @returns Markdown formatted string
   */
  private renderJsonTemplateToMarkdown(template: JsonTemplate, language: Language, variables?: Record<string, string>): string {
    const lines: string[] = [];
    
    // Add title
    const title = template.metadata.name[language] || template.metadata.name['en'] || Object.values(template.metadata.name)[0];
    lines.push(`# ${title}\n`);
    
    // Add sections
    for (const [sectionId, section] of Object.entries(template.content.sections)) {
      const sectionContent = this.renderJsonSection(section, language, variables, template.content.placeholders);
   ...
   * @returns Markdown formatted string for the section
   */
  private renderJsonSection(section: any, language: Language, variables?: Record<string, string>, placeholders?: Record<string, string>): string {
    const lines: string[] = [];
    ...
    if (section.content) {
      const content = section.content[language] || section.content['en'] || Object.values(section.content)[0];
      if (content) {
