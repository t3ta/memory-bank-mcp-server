...
export class ProgressConverter extends BaseDocumentTypeConverter {
  ...
  protected convertContent(document: JsonDocument, builder: MarkdownBuilder): void {
    ...
    if (content.workingFeatures && Array.isArray(content.workingFeatures) && content.workingFeatures.length > 0) {
      builder.list(content.workingFeatures);
    }
    
    // Pending implementation section
    builder.heading('未実装の機能', 2);
    if (content.pendingImplementation && Array.isArray(content.pendingImplementation) && content.pendingImplementation.length > 0) {
    ...
    if (content.knownIssues && Array.isArray(content.knownIssues) && content.knownIssues.length > 0) {
