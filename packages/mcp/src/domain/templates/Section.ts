...
export class Section {
  ...
  getTitle(language: Language): string {
    ...
    const title = this._titleMap[language.code];
    if (title !== undefined) {
      return title;
    }
    ...
    // Ensure a string is always returned, even if undefined (shouldn't happen with validation)
    return this._titleMap[firstKey] ?? ''; // Already ensures string return
  }
  ...
  getContent(language: Language): string {
    ...
    const content = this._contentMap[language.code];
    ...
    if (contentKeys.length > 0) {
      ...
      // Ensure a string is always returned, even if undefined
      return this._contentMap[firstKey] ?? '';
    }
// No content available, return empty string
return '';
}
