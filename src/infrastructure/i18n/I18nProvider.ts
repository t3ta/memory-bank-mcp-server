...
import { Language, TranslationKey, isValidLanguage, TranslationFile } from '../../schemas/v2/i18n-schema.js';
...
export class I18nProvider implements II18nProvider {
  ...
  translate(key: TranslationKey, language: Language, params?: Record<string, string>): string {
    ...
    if (!this.translations.has(language)) {
      console.warn(`Translations not loaded for language ${language}, falling back to ${this.defaultLanguage}`);
    ...
    if (!translation && language !== this.defaultLanguage && this.translations.has(this.defaultLanguage)) {
  ...
  async loadTranslations(language: Language): Promise<boolean> {
    try {
      ...
      if (translationFile.language !== language) {
        console.warn(`Language mismatch in translation file: expected ${language}, got ${translationFile.language}`);
        return false;
      }
