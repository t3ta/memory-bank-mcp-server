...
`;
    ...
    mockBranchRepository.getDocument.mockImplementation(async (branchInfo: BranchInfo, path: DocumentPath) => {
    ...
    expect(result.files.activeContext?.activeDecisions).toEqual(['テストフレームワークとしてJestを使用']);
    ...
    expect(result.files.systemPatterns?.technicalDecisions?.[0].context).toBe('テストフレームワークを選択する必要がある');
    expect(result.files.systemPatterns?.technicalDecisions?.[0].decision).toBe('Jestを使用する');
    expect(result.files.systemPatterns?.technicalDecisions?.[0].consequences).toEqual(['TypeScriptとの統合が良い', 'モック機能が充実']);
  });
...
`;
    ...
    expect(result.files.systemPatterns?.technicalDecisions?.[1].context).toBe('ファイル配置の規則を定義する必要がある');
    expect(result.files.systemPatterns?.technicalDecisions?.[1].decision).toBe('クリーンアーキテクチャに従う');
    expect(result.files.systemPatterns?.technicalDecisions?.[1].consequences).toEqual(['関心の分離が明確', 'テスト可能性の向上']);
  });
